...
public class BackroomsChunkGenerator extends ChunkGenerator {
    ...
    public int getHeight(int x, int z, Heightmap.Type heightmap, HeightLimitView world,
                         NoiseConfig noiseConfig) { return 256; }
    ...
    private void generateFloor(Chunk chunk, int chunkStartX, int chunkStartZ, int floorStartY) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                chunk.setBlockState(new BlockPos(x, floorY, z), Blocks.YELLOW_TERRACOTTA.getDefaultState(), false);
                chunk.setBlockState(new BlockPos(x, ceilingY, z), ModBlocks.BACKBOARD_BLOCK.getDefaultState(), false);
    ...
    private void generateBacteriaEcosystem(Chunk chunk, int chunkStartX, int chunkStartZ,
        ...
        long bacteriaSeed = ((long) chunkStartX * 341873128712L +
                (long) chunkStartZ * 132897987541L + floorStartY * 555555L) ^ worldSeed;
        ChunkRandom bacteriaRandom = new ChunkRandom(new CheckedRandom(bacteriaSeed));

        if (bacteriaRandom.nextFloat() >= CONFIG.generationBacteriaClusterChance) return;
        ...
        for (GridPos existing : floorGrids) {
            if (Math.abs(thisGrid.x - existing.x) <= 16 && Math.abs(thisGrid.z - existing.z) <= 16 && existing.y == thisGrid.y) return;
        }
        floorGrids.add(thisGrid);

        int centerX = bacteriaRandom.nextInt(10) + 3;
        int centerZ = bacteriaRandom.nextInt(10) + 3;
        int clusterSize = bacteriaRandom.nextInt(3) + 3;
        ...
        for (int dx = -clusterSize; dx <= clusterSize; dx++) {
            for (int dz = -clusterSize; dz <= clusterSize; dz++) {
                ...
                if (bacteriaRandom.nextFloat() < vineChance) placeVinesOnNearbyWalls(chunk, x, z, wallBottomY, wallTopY, bacteriaRandom);
            }
        }
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                if (!chunk.getBlockState(floorPos).isSolid() || chunk.getBlockState(floorPos).isAir()) continue;
                if (bacteriaRandom.nextFloat() < 0.5f) {
                    BlockPos shroomPos = new BlockPos(x, wallBottomY, z);
                    if (chunk.getBlockState(shroomPos).isAir()) {
                        chunk.setBlockState(shroomPos, ModBlocks.BACTERIA_SHROOM_HORIZONTAL.getDefaultState()
                                .with(Properties.HORIZONTAL_FACING, Direction.Type.HORIZONTAL.random(bacteriaRandom)), false);
        ...
        for (int dx = -clusterSize; dx <= clusterSize; dx++) {
            for (int dz = -clusterSize; dz <= clusterSize; dz++) {
                ...
                if (bacteriaRandom.nextFloat() < 0.15f) placeVerticalShroomOnWall(chunk, x, z, wallBottomY, wallTopY, bacteriaRandom);
            }
        }
    }
    ...
    private void placeVinesOnNearbyWalls(Chunk chunk, int x, int z, int wallBottomY, int wallTopY, ChunkRandom random) {
        ...
        for (Direction dir : directions) {
            ...
            if (wallPos.getX() < 0 || wallPos.getX() >= 16 || wallPos.getZ() < 0 || wallPos.getZ() >= 16) continue;
            if (isWallpaperBlock(chunk.getBlockState(wallPos))) {
    ...
    private void placeVerticalShroomOnWall(Chunk chunk, int x, int z, int wallBottomY, int wallTopY, ChunkRandom random) {
        ...
        for (Direction dir : directions) {
            ...
            if (isWallpaperBlock(chunk.getBlockState(wallPos))) {
                ...
                if (chunk.getBlockState(shroomPos).isAir()) {
                    chunk.setBlockState(shroomPos, ModBlocks.BACTERIA_SHROOM_VERTICAL.getDefaultState()
                            .with(Properties.HORIZONTAL_FACING, dir), false);
                    decayWallpaperAt(chunk, wallPos, random);
                    return;
                }
            }
        ...
        for (int dx = -2; dx <= 2; dx++) {
            for (int dz = -2; dz <= 2; dz++) {
                ...
                for (int y = wallBottomY; y <= wallTopY; y++) {
                    ...
                    if (isWallpaperBlock(chunk.getBlockState(pos)) && random.nextFloat() < 0.15f) decayWallpaperAt(chunk, pos, random);
                }
            }
        }
    }
    ...
    private void decayWallpaperAt(Chunk chunk, BlockPos pos, ChunkRandom random) {
        ...
        if (roll < 0.25f) newState = getWetVariant(currentState);
        else if (roll < 0.45f) newState = getStainedVariant(currentState);
        else if (roll < 0.60f) newState = getMoldyVariant(currentState);
        else if (roll < 0.70f) newState = getMoldInfectedVariant(currentState);
        if (newState != currentState) chunk.setBlockState(pos, newState, false);
    }

    private BlockState getWetVariant(BlockState state) {
        Block block = state.getBlock();
        if (block == ModBlocks.WALLPAPER_BLOCK || block == ModBlocks.STAINED_WALLPAPER_BLOCK) return ModBlocks.WET_WALLPAPER_BLOCK.getDefaultState();
        if (block == ModBlocks.TORN_WALLPAPER_BLOCK || block == ModBlocks.STAINED_TORN_WALLPAPER_BLOCK) return ModBlocks.WET_TORN_WALLPAPER_BLOCK.getDefaultState();
        return state;
    }

    private BlockState getStainedVariant(BlockState state) {
        Block block = state.getBlock();
        if (block == ModBlocks.WALLPAPER_BLOCK) return ModBlocks.STAINED_WALLPAPER_BLOCK.getDefaultState();
        if (block == ModBlocks.TORN_WALLPAPER_BLOCK) return ModBlocks.STAINED_TORN_WALLPAPER_BLOCK.getDefaultState();
        return state;
    }

    private BlockState getMoldyVariant(BlockState state) {
        Block block = state.getBlock();
        if (isRegularWallpaper(block)) return ModBlocks.MOLDY_WALLPAPER_BLOCK.getDefaultState();
        if (isTornWallpaper(block)) return ModBlocks.MOLDY_TORN_WALLPAPER_BLOCK.getDefaultState();
        return state;
    }

    private BlockState getMoldInfectedVariant(BlockState state) {
        Block block = state.getBlock();
        if (isRegularWallpaper(block)) return ModBlocks.MOLD_INFECTED_WALLPAPER_BLOCK.getDefaultState();
        if (isTornWallpaper(block)) return ModBlocks.MOLD_INFECTED_TORN_WALLPAPER_BLOCK.getDefaultState();
        return state;
    }
    ...
    private void generateProps(Chunk chunk, int chunkStartX, int chunkStartZ,
                               int floorStartY, int floorY, int wallBottomY, int wallTopY) {
        long propSeed = ((long) chunkStartX * 341873128712L + (long) chunkStartZ * 132897987541L + floorStartY * 789012345L) ^ worldSeed;
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                if (chunk.getBlockState(new BlockPos(x, floorY, z)).isAir()) continue;
                if (!chunk.getBlockState(new BlockPos(x, wallBottomY, z)).isAir()) continue;
                Direction wallDirection = getAdjacentWallDirection(chunk, x, z, wallBottomY);
                if (wallDirection != null) wallPositions.add(new PropPosition(x, z, wallDirection));
                else openPositions.add(new PropPosition(x, z, null));
            }
        }
        ...
        if (!wallPositions.isEmpty() && propRandom.nextFloat() < CONFIG.generationLockerChance) {
        ...
        if (propRandom.nextFloat() < CONFIG.generationWaterCoolerChance) {
            List<PropPosition> coolerPositions = new ArrayList<>();
            coolerPositions.addAll(openPositions); coolerPositions.addAll(wallPositions);
        ...
        if (propRandom.nextFloat() < CONFIG.generationBarrelChance) {
            List<PropPosition> barrelPositions = new ArrayList<>();
            barrelPositions.addAll(openPositions); barrelPositions.addAll(wallPositions);
            ...
            for (int i = 0; i < barrelCount && i < barrelPositions.size(); i++) {
                ...
                Direction facing = pos.wallDirection != null ? pos.wallDirection : Direction.fromHorizontal(propRandom.nextInt(4));
    ...
    private void generateCreepyWallDecor(Chunk chunk, int chunkStartX, int chunkStartZ,
        ...
        long decorSeed = ((long) chunkStartX * 341873128712L + (long) chunkStartZ * 132897987541L + floorStartY * 888888L) ^ worldSeed;
        ChunkRandom decorRandom = new ChunkRandom(new CheckedRandom(decorSeed));
        if (decorRandom.nextFloat() >= CONFIG.generationCreepySignChance) return;

        int x = decorRandom.nextInt(14) + 1, z = decorRandom.nextInt(14) + 1;
        BlockPos signPos = new BlockPos(x, wallBottomY, z);
        if (chunk.getBlockState(signPos).isAir()) {
            BlockState signState = Blocks.DARK_OAK_SIGN.getDefaultState().with(Properties.ROTATION, decorRandom.nextInt(16));
            ...
            if (signEntity instanceof SignBlockEntity sign) {
                ...
                for (int i = 0; i < 4; i++) signText = signText.withMessage(i, Text.literal(messages[i]));
    ...
    private void generateMossPatch(Chunk chunk, int chunkStartX, int chunkStartZ,
        ...
        long mossSeed = ((long) chunkStartX * 341873128712L + (long) chunkStartZ * 132897987541L + floorStartY * 777777L) ^ worldSeed;
        ChunkRandom mossRandom = new ChunkRandom(new CheckedRandom(mossSeed));
        if (mossRandom.nextFloat() >= CONFIG.generationMossPatchChance) return;

        int centerX = mossRandom.nextInt(10) + 3, centerZ = mossRandom.nextInt(10) + 3;
        int patchSize = mossRandom.nextInt(3) + 2;
        for (int dx = -patchSize; dx <= patchSize; dx++) {
            for (int dz = -patchSize; dz <= patchSize; dz++) {
                int x = centerX + dx, z = centerZ + dz;
                if (x < 0 || x >= 16 || z < 0 || z >= 16) continue;
                double dist = Math.sqrt(dx * dx + dz * dz) + (mossRandom.nextFloat() - 0.5f) * 2.0f;
                if (dist > patchSize) continue;
                BlockPos floorPos = new BlockPos(x, floorY, z), abovePos = new BlockPos(x, wallBottomY, z);
                if (dist < patchSize * 0.6f && chunk.getBlockState(floorPos).isOf(Blocks.YELLOW_TERRACOTTA)) {
                    chunk.setBlockState(floorPos, Blocks.MOSS_BLOCK.getDefaultState(), false);
                    float propRoll = mossRandom.nextFloat();
                    if (propRoll < 0.30f && chunk.getBlockState(abovePos).isAir()) chunk.setBlockState(abovePos, Blocks.MOSS_CARPET.getDefaultState(), false);
                    else if (propRoll < 0.55f) placeLichenOnNearbyWalls(chunk, x, wallBottomY, z, mossRandom);
                    else if (propRoll < 0.70f && chunk.getBlockState(abovePos).isAir()) chunk.setBlockState(abovePos, (mossRandom.nextBoolean() ? Blocks.BROWN_MUSHROOM : Blocks.RED_MUSHROOM).getDefaultState(), false);
                    else if (propRoll < 0.80f && chunk.getBlockState(abovePos).isAir()) chunk.setBlockState(abovePos, Blocks.AZALEA.getDefaultState(), false);
                } else if (dist < patchSize * 0.8f && chunk.getBlockState(floorPos).isOf(Blocks.YELLOW_TERRACOTTA) && mossRandom.nextFloat() < 0.3f && chunk.getBlockState(abovePos).isAir()) {
                    chunk.setBlockState(abovePos, Blocks.MOSS_CARPET.getDefaultState(), false);
                }
            }
        }
        if (mossRandom.nextFloat() < 0.08f) {
            int skullX = centerX + mossRandom.nextInt(3) - 1, skullZ = centerZ + mossRandom.nextInt(3) - 1;
            if (skullX >= 0 && skullX < 16 && skullZ >= 0 && skullZ < 16) {
                ...
                if (skullBlock.isAir() || skullBlock.isOf(Blocks.MOSS_CARPET) || skullBlock.isOf(Blocks.AZALEA))
                    chunk.setBlockState(skullPos, Blocks.SKELETON_SKULL.getDefaultState().with(Properties.ROTATION, mossRandom.nextInt(16)), false);
            }
        }
    }
    ...
    private void placeLichenOnNearbyWalls(Chunk chunk, int x, int y, int z, ChunkRandom random) {
        for (Direction dir : Direction.Type.HORIZONTAL) {
            for (int dist = 1; dist <= 2; dist++) {
                ...
                if (checkPos.getX() < 0 || checkPos.getX() >= 16 || checkPos.getZ() < 0 || checkPos.getZ() >= 16) continue;
                if (chunk.getBlockState(checkPos).isSolid() && isWallpaperBlock(chunk.getBlockState(checkPos))) {
    ...
    private boolean isWallpaperBlock(BlockState state) {
        return state.isOf(ModBlocks.WALLPAPER_BLOCK) || state.isOf(ModBlocks.STAINED_WALLPAPER_BLOCK) ||
                state.isOf(ModBlocks.WET_WALLPAPER_BLOCK) || state.isOf(ModBlocks.MOLDY_WALLPAPER_BLOCK) ||
                state.isOf(ModBlocks.MOLD_INFECTED_WALLPAPER_BLOCK) || state.isOf(ModBlocks.SPONGE_WALLPAPER_BLOCK) ||
                state.isOf(ModBlocks.TORN_WALLPAPER_BLOCK) || state.isOf(ModBlocks.STAINED_TORN_WALLPAPER_BLOCK) ||
                state.isOf(ModBlocks.WET_TORN_WALLPAPER_BLOCK) || state.isOf(ModBlocks.MOLDY_TORN_WALLPAPER_BLOCK) ||
                state.isOf(ModBlocks.MOLD_INFECTED_TORN_WALLPAPER_BLOCK);
    }

    private String[] getCreepyMessage(ChunkRandom random) {
        String[][] messages = {
                {"", "DON'T", "LOOK", ""}, {"", "HELP", "ME", ""}, {"", "RUN", "", ""},
                {"", "IT SEES", "YOU", ""}, {"", "NO", "EXIT", ""}, {"", "BEHIND", "YOU", ""},
                {"", "→", "", ""}, {"", "↓↓", "", ""}, {"", "SHE", "WATCHES", ""},
                {"", "WAKE", "UP", ""}, {"", "THE END", "IS NEAR", ""}, {"", "FOLLOW", "THE LIGHT", ""},
                {"", "TRUST", "NO ONE", ""}, {"", "DON'T", "TURN", "AROUND"}, {"", "IT'S", "HERE", ""},
                {"", "SAVE", "YOURSELF", ""}, {"", "DEAD", "END", ""}, {"", "GO", "BACK", ""}
        };
    ...
    private void generateHoles(Chunk chunk, int chunkStartX, int chunkStartZ, int floorStartY,
        ...
        long holeSeed = ((long) chunkStartX * 341873128712L + (long) chunkStartZ * 132897987541L + floorStartY * 111111111L) ^ worldSeed;
        ChunkRandom holeRandom = new ChunkRandom(new CheckedRandom(holeSeed));
        if (holeRandom.nextFloat() >= CONFIG.generationHolesChance) return;

        int holeType = holeRandom.nextInt(4);
        int holeX = holeRandom.nextInt(8) + 4, holeZ = holeRandom.nextInt(8) + 4;
        switch (holeType) {
            case 0: punchHole(chunk, holeX, holeZ, floorY, ceilingY, lightY); break;
            case 1: for (int dx = 0; dx < 2; dx++) for (int dz = 0; dz < 2; dz++) punchHole(chunk, holeX+dx, holeZ+dz, floorY, ceilingY, lightY); break;
            case 2: punchHole(chunk, holeX, holeZ, floorY, ceilingY, lightY); punchHole(chunk, holeX+1, holeZ, floorY, ceilingY, lightY); punchHole(chunk, holeX-1, holeZ, floorY, ceilingY, lightY); punchHole(chunk, holeX, holeZ+1, floorY, ceilingY, lightY); punchHole(chunk, holeX, holeZ-1, floorY, ceilingY, lightY); break;
            case 3: for (int dx = -3; dx <= 2; dx++) for (int dz = -3; dz <= 2; dz++) punchHole(chunk, holeX+dx, holeZ+dz, floorY, ceilingY, lightY); break;
        }
    }
    ...
    private void placeLocker(Chunk chunk, int x, int z, int floorY, Direction facing) {
        BlockState lockerState = ModBlocks.LOCKER.getDefaultState().with(Properties.HORIZONTAL_FACING, facing).with(Properties.OPEN, false);
        chunk.setBlockState(new BlockPos(x, floorY, z), lockerState.with(Properties.DOUBLE_BLOCK_HALF, net.minecraft.block.enums.DoubleBlockHalf.LOWER), false);
        chunk.setBlockState(new BlockPos(x, floorY+1, z), lockerState.with(Properties.DOUBLE_BLOCK_HALF, net.minecraft.block.enums.DoubleBlockHalf.UPPER), false);
    }
    ...
    private void placeWaterCooler(Chunk chunk, int x, int z, int floorY, Direction facing) {
        ...
        chunk.setBlockState(new BlockPos(x, floorY, z), ModBlocks.WATER_COOLER.getDefaultState().with(Properties.HORIZONTAL_FACING, coolerFacing), false);
    }

    private void placeBarrel(Chunk chunk, int x, int z, int floorY, Direction facing) {
        chunk.setBlockState(new BlockPos(x, floorY, z), Blocks.BARREL.getDefaultState().with(BarrelBlock.FACING, facing.getOpposite()), false);
    }

    private static class PropPosition {
        final int x, z; final Direction wallDirection;
        PropPosition(int x, int z, Direction wallDirection) { this.x = x; this.z = z; this.wallDirection = wallDirection; }
    }

    private void generateBoundaryWalls(Chunk chunk, int chunkStartX, int chunkStartZ, int floorStartY, int wallBottomY, int wallTopY) {
        long chunkSeed = ((long) chunkStartX * 341873128712L + (long) chunkStartZ * 132897987541L + floorStartY * 456789L) ^ worldSeed;
    ...
    private void generateInternalWalls(Chunk chunk, int chunkStartX, int chunkStartZ, int floorStartY, int wallBottomY, int wallTopY) {
        long chunkSeed = ((long) chunkStartX * 341873128712L + (long) chunkStartZ * 132897987541L + floorStartY * 987654L) ^ worldSeed;
        ChunkRandom chunkRandom = new ChunkRandom(new CheckedRandom(chunkSeed));
        if (chunkRandom.nextFloat() >= CONFIG.generationInternalWallChance) return;
        int wallCount = chunkRandom.nextInt(2) + 1;
        for (int i = 0; i < wallCount; i++) generateRandomWallShape(chunk, chunkStartX, chunkStartZ, wallBottomY, wallTopY, chunkRandom, floorStartY);
    }

    private void generateRandomWallShape(Chunk chunk, int chunkStartX, int chunkStartZ, int wallBottomY, int wallTopY, ChunkRandom random, int floorStartY) {
        ...
        switch (wallType) {
            ...
            case 2: int leg = length/2; for (int dx = 0; dx < leg && (startX+dx)<15; dx++) placeWallBlock(chunk, startX+dx, startZ, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); for (int dz = 0; dz < leg && (startZ+dz)<15; dz++) placeWallBlock(chunk, startX, startZ+dz, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); break;
            case 3: int arm = length/4; for (int dx = -arm; dx <= arm; dx++) { int px=startX+dx; if (px>0&&px<15) placeWallBlock(chunk, px, startZ, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); } for (int dz = -arm; dz <= arm; dz++) { int pz=startZ+dz; if (pz>0&&pz<15) placeWallBlock(chunk, startX, pz, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); } break;
            case 4: int top = length/3; for (int dx = -top; dx <= top; dx++) { int px=startX+dx; if (px>0&&px<15) placeWallBlock(chunk, px, startZ, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); } for (int dz = 0; dz < length-top && (startZ+dz)<15; dz++) placeWallBlock(chunk, startX, startZ+dz, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); break;
            case 5: int blen = length/2; for (int dx = 0; dx < blen && (startX+dx)<15; dx++) { placeWallBlock(chunk, startX+dx, startZ, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); if (startZ+blen<15) placeWallBlock(chunk, startX+dx, startZ+blen, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); } for (int dz = 0; dz < blen && (startZ+dz)<15; dz++) placeWallBlock(chunk, startX, startZ+dz, wallBottomY, wallTopY, chunkStartX, chunkStartZ, floorStartY); break;
        }
    }

    private void generateColumns(Chunk chunk, int chunkStartX, int chunkStartZ, int floorStartY, int wallBottomY, int wallTopY) {
        long chunkSeed = ((long) chunkStartX * 341873128712L + (long) chunkStartZ * 132897987541L + floorStartY * 123456L) ^ worldSeed;
        ChunkRandom chunkRandom = new ChunkRandom(new CheckedRandom(chunkSeed));
        if (chunkRandom.nextFloat() >= CONFIG.generationColumnChance) return;
        ...
        for (int i = 0; i < columnCount; i++) {
            ...
            int size = sizeRoll < 0.4f ? 1 : sizeRoll < 0.7f ? 2 : sizeRoll < 0.9f ? 3 : 4;
            for (int dx = -size/2; dx <= size/2; dx++) {
                for (int dz = -size/2; dz <= size/2; dz++) {
                    int px = centerX+dx, pz = centerZ+dz;
                    if (px >= 0 && px < 16 && pz >= 0 && pz < 16) {
                        for (int y = wallBottomY; y <= wallTopY; y++) {
                            chunk.setBlockState(new BlockPos(px, y, pz), getWallBlockForY(chunkStartX+px, chunkStartZ+pz, floorStartY, y, wallBottomY), false);
                        }
                    }
                }
            }
    ...
    private void placeWallBlock(Chunk chunk, int x, int z, int wallBottomY, int wallTopY, int chunkStartX, int chunkStartZ, int floorStartY) {
        for (int y = wallBottomY; y <= wallTopY; y++)
            chunk.setBlockState(new BlockPos(x, y, z), getWallBlockForY(chunkStartX+x, chunkStartZ+z, floorStartY, y, wallBottomY), false);
    }
    ...
    private BlockState getLightPatternBlock(int localX, int localZ, int worldX, int worldZ) {
        if ((localZ == 7 || localZ == 8) && (localX == 3 || localX == 11)) return getLightBlockWithCorruption(worldX, worldZ);
    ...
    private BlockState getLightBlockWithCorruption(int worldX, int worldZ) {
        ...
        if (darkZoneNoise > 0.6) return lightRandom.nextFloat() < 0.9f ? Blocks.BONE_BLOCK.getDefaultState() : Blocks.OCHRE_FROGLIGHT.getDefaultState();
        else return lightRandom.nextFloat() < 0.1f ? Blocks.BONE_BLOCK.getDefaultState() : Blocks.OCHRE_FROGLIGHT.getDefaultState();
    }
    ...
    private BlockState getOrganicWallBlock(int worldX, int worldZ, int floorY) {
        ...
        if (spongeRandom.nextFloat() < CONFIG.generationSpongeWallpaperChance) return ModBlocks.SPONGE_WALLPAPER_BLOCK.getDefaultState();
        ...
        for (MoldCluster cluster : clusters) {
            ...
            if (distance < cluster.radius) {
                double adjustedDistance = distance + getMoldNoise(worldX, worldZ, cluster.seed) * 1.5;
    ...
    private MoldCluster[] getMoldClustersForRegion(int worldX, int worldZ) {
        ...
        for (int i = 0; i < numClusters; i++) {
            clusters[i] = new MoldCluster(regionX * 64 + regionRandom.nextInt(64), regionZ * 64 + regionRandom.nextInt(64),
                    regionRandom.nextInt(2) + 2, regionRandom.nextLong());
        }
    ...
    private double getMoldNoise(int worldX, int worldZ, long seed) {
        return Math.sin(worldX * 0.3 + seed) * Math.cos(worldZ * 0.3 - seed * 0.7) * 0.5;
    }

    private static class MoldCluster {
        final int centerX, centerZ, radius; final long seed;
        MoldCluster(int centerX, int centerZ, int radius, long seed) { this.centerX = centerX; this.centerZ = centerZ; this.radius = radius; this.seed = seed; }
    }
}