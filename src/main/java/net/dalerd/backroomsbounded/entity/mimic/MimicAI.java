...
public class MimicAI extends Goal {
    ...
    private int sneakToggleCooldown = 0;
    ...
    public void tick() {
        ...
        if (sneakToggleCooldown > 0) sneakToggleCooldown--;
        if (lookAroundCooldown > 0) lookAroundCooldown--;
        if (commandTimer > 0) {
            commandTimer--;
        ...
        if (mimic.isAggressive()) {
            ...
            if (distance > 25) mimic.getNavigation().startMovingTo(targetPlayer, 0.7);
            else if (distance < 9) {
                Vec3d away = mimic.getPos().subtract(targetPlayer.getPos()).normalize().multiply(0.5);
                mimic.getNavigation().startMovingTo(mimic.getX() + away.x, mimic.getY(), mimic.getZ() + away.z, 0.5);
            }
            mimic.getLookControl().lookAt(targetPlayer);
            ...
            if (actionCooldown <= 0 && distance < 8) {
                mimicContainerInteraction();
                actionCooldown = 100 + random.nextInt(200);
            }
        }
    }

    private void playerLikeBehaviors() {
        if (sneakToggleCooldown <= 0 && random.nextFloat() < 0.02f) {
            mimic.setSneaking(!mimic.isSneaking());
            sneakToggleCooldown = 60 + random.nextInt(120);
        }
        if (lookAroundCooldown <= 0 && mimic.getNavigation().isIdle() && targetPlayer == null) {
            targetYaw = mimic.getYaw() + random.nextInt(180) - 90;
            lookAroundCooldown = 40 + random.nextInt(60);
        }
        if (lookAroundCooldown > 0 && mimic.getNavigation().isIdle()) {
            mimic.setYaw(targetYaw);
        }
    }
    ...
    private void pickUpNearbyItems() {
        ...
        List<ItemEntity> items = mimic.getWorld().getEntitiesByClass(ItemEntity.class,
                new Box(targetPlayer.getBlockPos()).expand(5), item -> true);
        if (!items.isEmpty()) {
